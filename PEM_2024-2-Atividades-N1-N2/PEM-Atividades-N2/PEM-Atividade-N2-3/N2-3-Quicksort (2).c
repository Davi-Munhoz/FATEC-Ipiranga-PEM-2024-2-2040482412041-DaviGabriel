...
*--------------------------------------------------------*
* Objetivo do Programa: Mostrando o Estado da memoria    *
* Data - 18/10/2024                                               * 
* Autor: Davi Gabriel Ribeiro Munhoz*
*--------------------------------------------------------*/
#include <stdio.h>

int momento = 1; // Contador global para identificar os momentos

// Função para trocar os valores de duas posições do array
void troca(int *a, int *b) {
    if (*a != *b) {  // Verifica se é necessário trocar
        printf("\n%dº Momento: Trocando %d com %d\n", momento++, *a, *b);
        int temp = *a;
        *a = *b;
        *b = temp;
    }
...
int particiona(int arr[], int baixo, int alto) {
    ...
    printf("\n%dº Momento: Estado da memória antes da partição\n", momento++);
    printf("Pivô: %d, Baixo: %d, Alto: %d\n", pivô, baixo, alto);
    printf("Array atual: ");
    ...
    printf("\n%dº Momento: Array após particionamento\n", momento++);
    for (int k = 0; k <= alto; k++) {
        printf("%d ", arr[k]);
    }
    printf("\n");
